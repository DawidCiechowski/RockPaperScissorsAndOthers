...
class Game:
    NEW_LINE = "\n"
    HINT = f"Choose one: \n{NEW_LINE.join([f'{option.name} - {index + 1}' for index, option in enumerate(Options)])}\n\nPress 'Q' and 'Enter' to quite a game.\n"
    RULES = {
        Options.ROCK: [
            Options.LIZARD,
            Options.WIZARD,
    ...
    def __init__(self) -> None:
        ...
        self.data = {"PC": 0, "YOU": 0, "TIES": 0}

    def evaluate(
        self, human: Options, pc: Options
    ) -> Tuple[str, Optional[Options], Optional[Options]]:
        """Return the string with the result and the winner and loser. Or none if it's a tie.

        Args:
            human (Options): Enum option chosen by a player
            pc (Options): Enum option generated by PC

        Returns:
            Tuple[str, Optional[Options], Optional[Options]]: Result, winner/None, loser/None
        """
    ...
    def play(self) -> None:
        ...
        def is_input_invalid(user_input: str) -> bool:
            return not user_input.isdigit() and user_input.lower() not in [
                "q",
                "rules",
                "r",
            ]
        ...
        while True:
            ...
            result, winner, loser = self.evaluate(choice, computers_choice)

            print(
                "TIE"
                if result == "TIE"
                else f"Winner: {result}. {winner.name} beats {loser.name}"
            )
